...
Create git version tags for a Python Poetry project.
...
current_tag=""
for commit_hash in $(git log --format="%H" --reverse -- pyproject.toml); do
    version_line=$(git show "$commit_hash":pyproject.toml | grep '^version = ' || true)
